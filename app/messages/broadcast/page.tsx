...
export default function BroadcastPage() {
  ...
  useEffect(() => {
    if (!shopId || !supabase) { setLoading(false); return; }
    void (async () => {
      ...
      const { data: biz } = await supabase
        ...
        .eq("id", shopId)
        .maybeSingle();
      const mode = (biz as { connection_mode?: string } | null)?.connection_mode as "cloud_api" | "whatsapp_web" | undefined;
      setConnectionMode(mode ?? "cloud_api");

      // Only fetch templates for cloud_api businesses
  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-6 px-4 py-6">
      ...
      {/* Manage Templates */}
      <div className={`rounded-2xl border border-[var(--color-border-card)] p-5 relative ${connectionMode === "whatsapp_web" ? "opacity-40 pointer-events-none select-none" : ""}`} style={{ background: "var(--color-surface-solid)" }}>
        {connectionMode === "whatsapp_web" && (
          <div className="absolute inset-0 flex items-center justify-center z-10 rounded-2xl" style={{ background: "rgba(0,0,0,0.3)" }}>
            <p className="text-sm font-medium text-white bg-black/60 rounded-lg px-4 py-2">Only available for Meta API businesses</p>
          </div>
