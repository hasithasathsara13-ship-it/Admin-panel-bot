...
const DEFAULTS: PlanRow[] = [
  { id: "Starter", display_name: "Starter", description: "For small shops", monthly_price_lkr: 4900, included_messages: 1000, max_products: 50, max_orders_per_month: null, features: [], is_active: true },
  { id: "Growth", display_name: "Growth", description: "For growing teams", monthly_price_lkr: 9900, included_messages: 3000, max_products: 200, max_orders_per_month: null, features: [], is_active: true },
  { id: "Scale", display_name: "Scale", description: "For high-volume stores", monthly_price_lkr: 19900, included_messages: 6000, max_products: null, max_orders_per_month: null, features: [], is_active: true },
];
...
export async function getPlans(): Promise<PlanRow[]> {
  ...
  try {
    const { data, error } = await admin
      .from("plans")
      .select("id, display_name, description, monthly_price_lkr, included_messages, max_products, max_orders_per_month, features, is_active")
      .eq("is_active", true)
